...
exports.index = function(whichId,canStop) {
    ...
    sh.select('abstract','id').from("content_preprints").where('id','=', whichId)
    .run()
    .then(abstract => {
       ...
       toDb.forEach( (element,index,array) => {
        ...
        sh.select('term','relevant_abstract').from('index_title').where('term','=','\''+element.t+'\'')
        .run()
        .then(returned => {
            ...
            if( Object.keys(returned).length !== 0 ) {
                ...
                if( scenario == true ) {
                    ...
                    .catch(e => {
                        ...
                        logger.error( sh.update('index_title').set('relevant_abstract','\''+JSON.stringify(relevant)+'\'')
                ...
                .catch(e => {
                    logger.error(e.toString());
                    logger.error( sh.insert({ term:element.t, relevant_abstract:'\'[{"p":"'+id+'","w":'+element.w+'}]\'' })
        ...
        .catch(e => {
            logger.error(e.toString());
            logger.error( sh.select('term','relevant_abstract').from('index_title').where('term','=',element.t).build() );
        });
       });
