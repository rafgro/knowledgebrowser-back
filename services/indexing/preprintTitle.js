...
exports.index = function(whichId,canStop) {
    ...
    loader.database.select('title','id').from("content_preprints").where('id','=', whichId)
    .run()
    .then(title => {
        ...
        */
        ...
        toDb.forEach( (element,index,array) => {
            ...
            loader.database.select('term','relevant').from('index_title').where('term','=',element.t)
            .run()
            .then(returned => {
                ...
                if( Object.keys(returned).length !== 0 ) {
                    ...
                    if( scenario == true ) {
                        ...
                        .catch(e => {
                            ...
                            logger.error( loader.database.update('index_title').set('relevant','\''+JSON.stringify(relevant)+'\'')
                    ...
                    .catch(e => {
                        logger.error(e.toString());
                        logger.error( loader.database.insert({ term:element.t, relevant:'\'[{"p":"'+id+'","w":'+element.w+'}]\'' })
            ...
            .catch(e => {
                logger.error(e.toString());
                logger.error( loader.database.select('term','relevant').from('index_title').where('term','=',element.t).build() );
            });
    ...
    .catch(e => {
        logger.error(e.toString());
        logger.error(loader.database.select('title','id').from("content_preprints").where('id','=', whichId).build());
    });
};