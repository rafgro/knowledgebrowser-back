...
exports.index = function(whichId,canStop) {
    ...
    loader.database.select('abstract','id').from("content_preprints").where('id','=', whichId)
    .run()
    .then(abstract => {

        if( abstract[0]["abstract"].length < 50 ) { return false; }

        var id = abstract[0]["id"];

        let abstractProper = striptags(unescape(abstract[0]["abstract"]).replace(/\r?\n|\r/g," ").replace(RegExp("\\$","g"),"")
        ...
        - append relative weight to each sentence (0.2 - 0.3 - 0.4) which will be multiplier of original title weight
        - index each sentence like in title algorithm

        */

       let nlpAbstract = nlp(abstractProper);
       let sentences = nlpAbstract.sentences().data().map(e=>{
           let tempRelWeight = 1;
           //in the future here work out sort of sentence importance analysis (like however or in addition should have lower importance)
       ...
       sentences.forEach( oneSentence => {
            let nlpSentence = nlp(oneSentence.text);
            ...
            // hyphenated terms
            nlpSentence.match("#Hyphenated").out('text').toString().split(" ").forEach(element => {
                if( element.length > 1 ) {
            ...
            // acronyms
            nlpSentence.acronyms().data().forEach(element => {
            ...
            nlpSentence.nouns().data().forEach(element => {
                if( element.normal.length > 1 ) {
                    ...
                    if( oneSentence.text.charAt( oneSentence.text.indexOf(element.text)-1 ) == '-' ) {
                        toDb.push( 
                            { t: getPreviousWord( oneSentence.text.substring( 0, oneSentence.text.indexOf(element.text) ) ) + element.text,
                            w: weight } );
                    }
                        ...
                        if( element.text.charAt(0) == ' ' ) {
                            toDb.push( 
                                { t: element.text.substring(1)
                                    + getNextWord( oneSentence.text.substring( oneSentence.text.indexOf(element.text) + element.text.length ) ),
                                w: weight } );
                        }
                        else {
                            toDb.push( 
            ...
            // single words
            nlpSentence.terms().data().forEach( element => {
       ...
       let clearedAbstract = nlp(abstractProper).normalize().sentences().out().replace(/\./g,"");
       toDb = toDb.map( element => {
           let wei = element.w;
           let howMany = (clearedAbstract.split(element.t)).length - 1;
       ...
       toDb.forEach( (element,index,array) => {
        ...
        loader.database.select('term','relevant_abstract').from('index_title').where('term','=','\''+element.t+'\'')
        .run()
        .then(returned => {
            ...
            if( Object.keys(returned).length !== 0 ) {
                ...
                if( scenario == true ) {
                    ...
                    loader.database.update('index_title')
                    .set('relevant_abstract','\''+JSON.stringify(relevant)+'\'')
                    ...
                    .catch(e => {
                        ...
                        logger.error( loader.database.update('index_title').set('relevant_abstract','\''+JSON.stringify(relevant)+'\'')
                ...
                loader.database.insert({
                    term:'\''+element.t+'\'',
                    relevant_abstract:'\'[{"p":"'+id+'","w":'+element.w+'}]\'' })
                .into('index_title')
                .run()
                .then(() => {
                ...
                .catch(e => {
                    logger.error(e.toString());
                    logger.error( loader.database.insert({ term:element.t, relevant_abstract:'\'[{"p":"'+id+'","w":'+element.w+'}]\'' })
        ...
        .catch(e => {
            logger.error(e.toString());
            logger.error( loader.database.select('term','relevant_abstract').from('index_title').where('term','=',element.t).build() );
        });
       });
