...
exports.start = function () {
    ...
    loader.database.select('value').from('manager').where('option','=', 'correcting_arxiv_offset')
    .run()
    .then(result => {

        loader.database.select('id','title','server').from("content_preprints").orderBy('id').limit(1000,result[0].value).run()
        .then( results => {
            ...
            if( results.length != 0 ) {
                ...
                loader.database.update('manager').set('value',value.toString()).where('option','=','correcting_arxiv_offset')
                .run()
                .then(() => {
