...
exports.start = function () {
    ...
    loader.database.select('value').from('manager').where('option','=', 'indexing_offset')
    .run()
    .then(result => {

        loader.database.select('id').from("content_preprints").orderBy('id').limit(20,result[0].value).run()
        .then( onepubresult => {
            ...
            if( onepubresult.length != 0 ) {
                ...
                loader.database.update('manager').set('value',value.toString()).where('option','=','indexing_offset')
                .run()
                .then(() => {
    ...
    loader.database.select('value').from('manager').where('option','=', 'indexing_offset_abstract')
    .run()
    .then(result => {
        ...
        .then( onepubresult => {
            ...
            if( onepubresult.length != 0 ) {
                let value = parseInt(result[0].value) +onepubresult.length;//+1;
                loader.database.update('manager').set('value',value.toString()).where('option','=','indexing_offset_abstract')
                .run()
                .then(() => {
                    logger.info('Offset set to '+value);
                })
